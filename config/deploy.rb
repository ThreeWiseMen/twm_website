...
default_environment['PATH'] = "/home/twm/ruby-1.8.7/bin:/usr/local/bin:/usr/kerberos/bin:/usr/local/bin:/bin:/usr/bin:/usr/X11R6/bin:/opt/CollabNet_Subversion/bin:/home/twm/bin"
...
namespace :deploy do
  ...
  task :start do
    run "cd #{current_path} && bundle exec $HOME/ruby-1.8.7/bin/unicorn_rails --config-file #{current_path}/config/unicorn.#{rails_env}.conf.rb --daemonize --env #{rails_env}"
  end
  task :restart do
